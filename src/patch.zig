pub fn main() !void {
    ...
    _: {
        const file: std.fs.File = .{
            .handle = try windows.OpenFile(try join(allocator, &.{ game, "index.html" }), .{
        ...
        const remaining = try reader.interface.allocRemaining(gpa, .unlimited);
    ...
    {
        const file: std.fs.File = .{
            .handle = try windows.OpenFile(try join(allocator, &.{ game, "patch.js" }), .{
